...
export interface CostOptions {
  /** Games to leave out from the start (Bathbot's `warmups`). */
  warmups: number;
...
export function analyzeMatch(games: readonly CostGame[], options: CostOptions): MatchAnalysis {
  ...
  const results: GameResult[] = games.map((game) => {
    ...
    if (format === "ffa" || !game.ended) return { id: game.id, counted: isCounted, winner: null, redScore: null, blueScore: null };
    ...
    return { id: game.id, counted: isCounted, winner, redScore, blueScore };
  });
