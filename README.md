...
| `GET /api/matches?…filters` | Paged matches. Filters: `q`, `sort` (`date`, `match_cost`, `maps`, `avg_score`, `accuracy`, `name`), `order`, `page`, `page_size`, `source` (`stable`, `lazer`), `with`, `vs` (comma-separated names or ids), `result` (`won`, `lost`), `played`, `tournament` (leaves out matchmaking lobbies), `hide_matchmaking` (ROMAI, ETX and o!mm bot lobbies), `min_cost`/`max_cost`, `min_maps`/`max_maps`, `date_from`/`date_to` |
| `GET /api/matches/:id` | One match: players with match costs, every map and score |
| `GET /api/matches/scores?…filters` | Tournament scores: the `/api/scores` filters plus `player` (`me`, `all`, a name or id), `match`, `source` and `hide_matchmaking` |
